...
void lavoroAuto(){
	...
	//~ perror("signal sem mutex pompalibera 117");
	if(p==1){
...
int main(){
	...
	while(i < numeroAuto){
		...
		if (pidRestituito < 0){
			...
			i++;
			//~ perror("ho appena fatto una fork riga 190");
			}
	}
	wait(NULL);
	wait(NULL);

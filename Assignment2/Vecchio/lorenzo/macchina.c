...
void lavoroAuto(){
	...
	waitSem(mutex_pompa1libera);	//mutex
		...
		//~ waitSem(idNumMa);
		//~ int numMacchineRimasteLocale = punt1[2];
	...
	if(p==1){	//cioè se la pompa1 è libera
		...
		//recupero il valore dell'importo dalla coda messaggi1
			...
			ricevi_msg(coda1,m);
			perror("AUTO  ricevi_messaggio 132");
			printf("\n importo da pagare ricevuto da pompa 1= %d \n",m->mtext);
